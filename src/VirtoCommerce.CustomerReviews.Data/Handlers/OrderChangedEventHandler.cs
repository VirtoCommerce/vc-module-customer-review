            ...
            using (var repository = _customerReviewRepository())
            {
                foreach (var jobArgument in jobArguments)
                {
                    ...
                    if (order != null)
                    {
                        ...
                        {
                            repository.Add(new RequestReviewEntity() { CreatedDate = DateTime.Now, CustomerOrderId = jobArgument.CustomerOrderId, ModifiedDate = DateTime.Now, EntityId = item.ProductId, EntityType = "Product", ReviewsRequest = 0, StoreId = jobArgument.StoreId, UserId = jobArgument.CustomerId });
                        }
                    }
                }
